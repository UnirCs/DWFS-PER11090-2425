    ...
    for (let i = 0; i < resultado.length; i++) {
        for (let j = 0; j < resultado[i].length; j++) {
            if (resultado[i][j].id === id) {
                ...
                return; // Salir de la función una vez que hemos encontrado la butaca
            }
        }
    }
    console.log(`Butaca con ID ${id} no encontrada.`);
}

// Marcar varias butacas como ocupadas en una sola línea
[5, 13, 25, 26, 28, 32, 33, 35, 38, 46, 50, 54, 58, 64, 63, 73, 75, 78, 81, 83, 84, 91, 92, 95, 100].forEach(id => ocuparButaca(id));
