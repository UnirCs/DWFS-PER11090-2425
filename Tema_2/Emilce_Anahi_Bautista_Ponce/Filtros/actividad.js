...
function redConverter() {
  ...
  //Aqui tu codigo
  // Modificar los canales G y B de todos los píxeles
...
function greenConverter() {
  ...
  //Aqui tu codigo
  // Modificar los canales R y B de todos los píxeles
  pixels.forEach(row => row.forEach(pixel => {
    pixel[0] = 0; // Rojo
    pixel[2] = 0; // Azul
  }));
...
function blueConverter() {
  ...
  //Aqui tu codigo
  // Modificar los canales R y G de todos los píxeles
  pixels.forEach(row => row.forEach(pixel => {
    pixel[0] = 0; // Rojo
    pixel[1] = 0; // Verde
  }));
...
function blackAndWhiteConverter() {
  ...
  pixels.forEach(row => row.forEach(pixel => {
    let average = Math.floor((pixel[0] + pixel[1] + pixel[2]) / 3);
    let color = average < 128 ? [0, 0, 0] : [255, 255, 255];
    pixel[0] = color[0];
    pixel[1] = color[1];
    pixel[2] = color[2];
  }));
...
function scaleDown() {
  ...
  //Aqui tu codigo
 // Reducción de tamaño de la imagen a la mitad
 let newPixels = [];
...
function merge(alphaFirst, alphaSecond) {
  ...
  //Aqui tu codigo
  row.map((pixel, j) => {
    let mergedR = Math.floor(alphaFirst * pixel[0] + alphaSecond * dogPixels[i][j][0]);
    let mergedG = Math.floor(alphaFirst * pixel[1] + alphaSecond * dogPixels[i][j][1]);
    let mergedB = Math.floor(alphaFirst * pixel[2] + alphaSecond * dogPixels[i][j][2]);
