...
function scaleDown() {
  ...
  for (let f = 0; f < pixels.length; f += 2) {
    let nuevaFila = [];
    for (let c = 0; c < pixels[f].length; c += 2) {
      nuevaFila.push(pixels[f][c]);
    }
    nP.push(nuevaFila);
  }
  handler.savePixels(nP, outputPath, handler.getShape()[0] / 2, handler.getShape()[1] / 2);
}
...
function dimBrightness(dimFactor) {
  ...
  for (let f=0; f<pixels.length; f++) {
    for (let c=0; c<pixels[f].length; c++) {
      pixels[f][c][0] = Math.round(pixels[f][c][0] / dimFactor);
      pixels[f][c][1] = Math.round(pixels[f][c][1] / dimFactor);
      pixels[f][c][2] = Math.round(pixels[f][c][2] / dimFactor);
    }
  }
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let f = 0; f < catPixels.length; f++) {
    ...
    for (let c = 0; c < catPixels[f].length; c++) {
      let pixel = [
        catPixels[f][c][0] * alphaFirst + dogPixels[f][c][0] * alphaSecond,
        catPixels[f][c][1] * alphaFirst + dogPixels[f][c][1] * alphaSecond,
        catPixels[f][c][2] * alphaFirst + dogPixels[f][c][2] * alphaSecond
      ];
      nuevaFila.push(pixel);
    }
