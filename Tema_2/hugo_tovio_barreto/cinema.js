...
function suggest(numSeats) {
    // Se valida que sea un número válido de butacas posibles
    ...
    for (let i = butacas.length - 1; i >= 0; i--) {
        ...
        for (let j = 0; j <= fila.length - numSeats; j++) {
            ...
            if (available) {
                ...
                for (let m = 0; m < numSeats; m++) {
                    // Marcamos los asientos como ocupados
                    fila[j + m].estado = true;
                    // Agregamos el ID del asiento al Set
                    selectedSeats.add(fila[j + m].id);
                }
