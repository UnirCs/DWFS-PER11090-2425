...
function redConverter() {
    ...
    // Recorrer todos los píxeles y poner los canales verde y azul a 0
    pixels.forEach((fila) => {
        fila.forEach((pixel) => {
            pixel[1] = 0; // Canal verde
            pixel[2] = 0; // Canal azul
        });
    });
...
function greyConverter() {
    ...
    pixels.forEach((fila) => {
        fila.forEach((pixel) => {
            let promedio = Math.round((pixel[0] + pixel[1] + pixel[2]) / 3);
            pixel[0] = promedio; // Asignamos el promedio al canal rojo
            pixel[1] = promedio; // Asignamos el promedio al canal verde
            pixel[2] = promedio; // Asignamos el promedio al canal azul
        });
    });
...
function scaleDown() {
    ...
    let scaledPixels = pixels.filter((fila, i) => i % 2 === 0) // Mantener filas pares
        .map((fila) => fila.filter((pixel, j) => j % 2 === 0)); // Mantener columnas pares
...
function invertColors() {
    ...
    pixels.forEach((fila) => {
        fila.forEach((pixel) => {
            pixel[0] = 255 - pixel[0]; // Canal rojo
            pixel[1] = 255 - pixel[1]; // Canal verde
            pixel[2] = 255 - pixel[2]; // Canal azul
        });
    });
...
function merge(alphaFirst, alphaSecond) {
    ...
    // Recorrer las filas y columnas de ambas imágenes y combinar los píxeles
    catPixels.forEach((fila, i) => {
        let mergedFila = [];
        fila.forEach((pixel, j) => {
            let catPixel = pixel;
            let dogPixel = dogPixels[i][j];
            // Sumar los valores de los píxeles y aplicar los factores de fusión
            let mergedPixel = [
            ...
            mergedFila.push(mergedPixel);
        });
        pixels.push(mergedFila);
    });
