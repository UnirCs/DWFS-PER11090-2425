...
function greyConverter() {
  ...
  for(let fila=0; fila<pixels.length;fila++){
    for(let columna=0; columna<pixels[fila].length; columna++){
      if (pixels[fila][columna].length === 0){
        //evitar division para cero
        console.log("Error en pixel");
        pixelMedia = 0;
      }else{
        //calculal la media
        pixelMedia = pixels[fila][columna].reduce((sum, num) => sum + num, 0) / pixels[fila][columna].length;
      }
      pixels[fila][columna][0] = pixelMedia;
      pixels[fila][columna][1] = pixelMedia;
      pixels[fila][columna][2] = pixelMedia;
    }
  }
...
function blackAndWhiteConverter() {
  ...
  for(let fila=0; fila<pixels.length;fila++){
    for(let columna=0; columna<pixels[fila].length; columna++){
      ...
      //condicion si el la media es meenor a 128, use color negro
      pixelBN = pixelMedia < 128 ? 0:255;
      pixels[fila][columna][0] = pixelBN;
      pixels[fila][columna][1] = pixelBN;
      pixels[fila][columna][2] = pixelBN;
    }
  }
...
function scaleDown() {
  ...
  const filteredRows = pixels.filter((_, rowIndex) => rowIndex % 2 !== 0);

  // For each remaining row, filter out columns with odd indices
  const filteredArray = filteredRows.map(row =>
      row.filter((_, colIndex) => colIndex % 2 !== 0)
  );

  handler.savePixels(filteredArray, outputPath, handler.getShape()[0] / 2, handler.getShape()[1] / 2);
}
...
function dimBrightness(dimFactor) {
  ...
  //if (dimFactor>0){
    ...
    const dimmedImage = pixels.map(layer =>
        layer.map(row =>
            row.map(value => value / dimFactor)
        )
    );
  //}

  handler.savePixels(dimmedImage, outputPath);
}
...
function invertColors() {
  ...
  // Subtract 255 from each element
  const invertedImage = pixels.map(layer =>
      layer.map(row =>
          row.map(value => 255 - value)
      )
  );

  handler.savePixels(invertedImage, outputPath);
}
...
function merge(alphaFirst, alphaSecond) {
  ...
  let pixels = [];
  // Verificar que las dimensiones de ambas imágenes coincidan
  if (catPixels.length !== dogPixels.length || catPixels[0].length !== dogPixels[0].length) {
    throw new Error("Las dimensiones de las imágenes no coinciden.");
  }

  // Fusionar las imágenes
  const mergedImage = catPixels.map((row, rowIndex) =>
      row.map((pixel, colIndex) =>
          pixel.map((channel, channelIndex) =>
              Math.round(
                  channel * alphaFirst +
                  dogPixels[rowIndex][colIndex][channelIndex] * alphaSecond
              )
          )
      )
  );
